...
_funds = player getVariable ["WL2_funds", 0];
_matesAvail = (WL2_matesAvailable + 1 - count units group player) max 0;
_servicesAvailable = WL2_sectorsArray # 5;

_purchase_income ctrlSetStructuredText parseText format ["<t size = '%7' align = 'center' shadow = '2'>%2 %3%4%5%6, " + localize "STR_A3_WL_max_group_size" + "</t>", _matesAvail, _funds, localize "STR_A3_WL_unit_cp", if ("A" in _servicesAvailable) then {", " + localize "STR_A3_WL_param32_title"} else {""}, if ("H" in _servicesAvailable) then {", " + localize "STR_A3_WL_module_service_helipad"} else {""}, if ("W" in _servicesAvailable) then {", " + localize "STR_A3_WL_param30_title"} else {""}, (1.5 call WL2_fnc_sub_purchaseMenuGetUIScale)];
...
for [{_i = 0}, {_i < lbSize _purchase_items}, {_i = _i + 1}] do {
	...
	_availability = call WL2_fnc_sub_purchaseMenuAssetAvailability;
...
if (count _assetDetails > 0) then {
	...
	if (_id == 6) then {
		_removeUnitsID = uiNamespace getVariable ["WL2_removeUnitsListID", -1];
	...
	if (_availability # 0 && ctrlEnabled _purchase_request) then {
		uiNamespace setVariable ["WL2_purchaseMenuItemAffordable", TRUE];
		if (uiNamespace getVariable ["WL2_purchaseMenuButtonHover", FALSE]) then {
			_color = WL2_colorFriendly;
		...
		uiNamespace setVariable ["WL2_purchaseMenuItemAffordable", FALSE];
	...
	_purchase_title_queue ctrlSetStructuredText parseText format ["<t size = '%2' align = 'center' shadow = '0'>%1</t>", if (count WL2_dropPool > 0) then {localize "STR_A3_WL_menu_airdrop_queue"} else {localize "STR_A3_WL_menu_airdrop_queue_empty"}, (1.25 call WL2_fnc_sub_purchaseMenuGetUIScale)];
	lbClear _purchase_queue;
	if (count WL2_dropPool > 0) then {
		{
			_x ctrlEnable TRUE;
			_x ctrlSetFade 0;
			_x ctrlCommit 0;
		...
		if (_funds >= WL2_dropCost && ctrlEnabled _purchase_request) then {
			uiNamespace setVariable ["WL2_purchaseMenuDropSectorAffordable", TRUE];
			if !(uiNamespace getVariable ["WL2_purchaseMenuButtonDropSectorHover", FALSE]) then {
				_purchase_drop_sector ctrlSetBackgroundColor _color;
				_purchase_drop_sector ctrlSetTextColor [1, 1, 1, 1];
			};
		} else {
			uiNamespace setVariable ["WL2_purchaseMenuDropSectorAffordable", FALSE];
		...
		if (_funds >= WL2_dropCost_far && WL2_vehsInBasket == ({(_x # 0) isKindOf "Thing"} count WL2_dropPool) && ctrlEnabled _purchase_request) then {
			uiNamespace setVariable ["WL2_purchaseMenuDropPlayerAffordable", TRUE];
			...
			if !(uiNamespace getVariable ["WL2_purchaseMenuButtonDropPlayerHover", FALSE]) then {
