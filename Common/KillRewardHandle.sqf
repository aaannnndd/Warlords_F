...
if !(isNull _instigator) then {
	if (local leader _instigator) then {
		...
		if (_responsibleLeader in WL2_allWarlords) then {
			...
			if (_killerSide != _unitSide && _unitSide in WL2_sidesArray) then {
				...
				[_responsibleLeader, _reward] call WL2_fnc_fundsControl;
			};
		};
	};
};