...
_asset addEventHandler ["Killed", {
	params ["_asset"];
	_asset setVariable ["WL2_deleteAt", WL_SYNCED_TIME + (if (_asset isKindOf "Man") then {WL2_corpseRemovalTimeout} else {WL2_wreckRemovalTimeout}), !isServer];
}];

if (isNull _owner && isServer) then {
	_asset spawn WL2_fnc_assetRelevanceCheck;
	_asset setSkill (0.2 + random 0.3);
};

if (isPlayer _owner) then {
	_asset setVariable ["WL2_ownerGrp", group _owner];
	_asset setVariable ["WL2_iconText", getText (configFile >> "CfgVehicles" >> typeOf _asset >> "displayName")];
	
	_friendlyFireProtection = _asset addEventHandler ["HandleDamage", {
		params ["_unit", "_selection", "_damage", "_source", "_projectile", "_hitIndex", "_instigator", "_hitPoint"];
		_ownerGrp = _unit getVariable ["WL2_ownerGrp", objNull];
	...
	if (_asset isKindOf "Man") then {
		_asset call WL2_fnc_sub_assetAssemblyHandle;
		
		_asset addEventHandler ["Fired", WL2_fnc_sub_restrictMines];
		_asset addEventHandler ["Killed", {
			WL2_matesAvailable = WL2_matesAvailable - 1;
			WL2_manLost = TRUE;
			[] spawn WL2_fnc_refreshOSD;
		}];
	} else {
		_asset setVariable ["WL2_icon", getText (configFile >> "CfgVehicles" >> typeOf _asset >> "Icon")];
		_asset setVariable ["WL2_nextRepair", 0];
		_asset setVariable ["WL2_nextRearm", 0];
		private _defaultMags = [];
		{
			_defaultMags pushBack (_asset magazinesTurret _x);
		} forEach allTurrets _asset;
		_asset setVariable ["WL2_defaultMagazines", _defaultMags];
		_ownedVehiclesVarName = format ["WL2_%1_ownedVehicles", getPlayerUID _owner];
		...
		if !(_asset isKindOf "StaticWeapon") then {
			WL2_recentlyPurchasedAssets pushBack _asset;
			...
			_asset spawn {
				...
				WL2_recentlyPurchasedAssets = WL2_recentlyPurchasedAssets - [_this];
			};
		};
		...
		_asset spawn {
			...
			while {alive _asset} do {
				...
				_repairCooldown = ((_asset getVariable "WL2_nextRepair") - WL_SYNCED_TIME) max 0;
				_rearmCooldown = ((_asset getVariable "WL2_nextRearm") - WL_SYNCED_TIME) max 0;
				
				if (_nearbyVehicles findIf {_x getVariable ["WL2_canRepair", FALSE]} != -1) then {
					if (_repairActionID == -1) then {
						_repairActionID = _asset addAction [
							"",
							{
								params ["_asset"];
								if ((_asset getVariable "WL2_nextRepair") <= WL_SYNCED_TIME) then {
							...
							FALSE,
							"",
							"alive _target && group _this == (_target getVariable ['WL2_ownerGrp', grpNull]) && vehicle _this == _this",
							WL_MAINTENANCE_RADIUS,
							FALSE
						];
						_asset setVariable ["WL2_repairActionID", _repairActionID];
					};
				...
				if (_nearbyVehicles findIf {_x getVariable ["WL2_canRearm", FALSE]} != -1) then {
					if (_rearmActionID == -1) then {
						_rearmActionID = _asset addAction [
							"",
							{
								params ["_asset"];
								if ((_asset getVariable "WL2_nextRearm") <= WL_SYNCED_TIME) then {
									_curWeapon = currentWeapon _asset;
									{
										private _turret = _x;
										private _mags = (_asset getVariable "WL2_defaultMagazines") # _forEachIndex;
										{
						...
						_asset setVariable ["WL2_rearmActionID", _rearmActionID];
					};
		...
		if !(_assembled || _asset isKindOf "Thing") then {
			...
			_asset call WL2_fnc_sub_vehicleLockAction;
		};
		...
		if (getNumber (configFile >> "CfgVehicles" >> typeOf _asset >> "transportRepair") > 0) then {_asset setRepairCargo 0; _asset setVariable ["WL2_canRepair", TRUE, TRUE]};
		if (getNumber (configFile >> "CfgVehicles" >> typeOf _asset >> "transportAmmo") > 0) then {_asset setAmmoCargo 0; _asset setVariable ["WL2_canRearm", TRUE, TRUE]};
	};
	...
	private _removeActionID = _asset addAction [
		"",
		{
			...
			publicVariableServer _ownedVehiclesVarName;
			(_this # 0) call WL2_fnc_sub_deleteAsset;
		},
		[],
		-20,
		FALSE,
		TRUE,
		"",
		"alive _target && vehicle _this != _target && group _this == (_target getVariable ['WL2_ownerGrp', grpNull])",
		-1,
		FALSE
	];
