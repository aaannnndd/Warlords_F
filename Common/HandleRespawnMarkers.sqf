...
switch (_mode) do {
	case "setup": {
		_spawnMarkers = {
			_sideID = ["west", "east", "guerrila"] select (WL2_sidesArray find _this);
			_respawnMarkerFormat = format ["respawn_%1_", _sideID];
			_base = _this call WL2_fnc_getSideBase;
			_baseSpots = _base call WL2_fnc_findSpawnPositions;
			...
			for [{_i = 1}, {_i <= WL_RESPAWN_MARKERS_CNT}, {_i = _i + 1}] do {
				_mrkr = createMarkerLocal [_respawnMarkerFormat + str _i, if (_baseSpotsCnt == WL_RESPAWN_MARKERS_CNT) then {_baseSpots # _i} else {selectRandom _baseSpots}];
			};
		};
		if (isServer) then {
			{_x call _spawnMarkers} forEach WL2_competingSides;
		} else {
			WL2_playerSide call _spawnMarkers;
		};
	};
	case "base_vulnerable": {
		_sideID = ["west", "east", "guerrila"] select (WL2_sidesArray find _side);
		_respawnMarkerFormat = format ["respawn_%1_", _sideID];
