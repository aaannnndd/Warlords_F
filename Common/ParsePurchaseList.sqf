...
private _arsenalHandled = if (WL2_arsenalEnabled) then {FALSE} else {TRUE};
	...
	if (isClass (_preset >> str _side)) then {
		{
			...
			if (_category == "Gear" && !_arsenalHandled) then {
				...
				_data pushBack ["Arsenal", WL2_arsenalCost, [], "< " + (localize "STR_A3_Arsenal") + " >", "\A3\Data_F_Warlords\Data\preview_arsenal.jpg", localize "STR_A3_WL_arsenal_open"];
			};
			
			if (_category == "Gear" && !_lastLoadoutHandled) then {
				_lastLoadoutHandled = TRUE;
				_data pushBack ["LastLoadout", WL2_lastLoadoutCost, [], "< " + (localize "STR_A3_WL_last_loadout") + " >", "\A3\Data_F_Warlords\Data\preview_loadout.jpg", localize "STR_A3_WL_last_loadout_info"];
			};
...
} forEach WL2_purchaseListTeplateArr;

_strategyArr = [];

if (WL2_scanEnabled) then {_strategyArr pushBack ["Scan", WL2_scanCost, [], localize "STR_A3_WL_param4_title", "\A3\Data_F_Warlords\Data\preview_scan.jpg", localize "STR_A3_WL_menu_scan_info"]};
if (WL2_fastTravelEnabled in [1, 2, 4]) then {_strategyArr pushBack ["FTSeized", WL2_fastTravelCostOwned, [], localize "STR_A3_WL_menu_fasttravel_seized", "\A3\Data_F_Warlords\Data\preview_ft_owned.jpg", localize "STR_A3_WL_menu_fasttravel_info"]};
if (WL2_fastTravelEnabled in [1, 2]) then {_strategyArr pushBack ["FTConflict", WL2_fastTravelCostContested, [], localize "STR_A3_WL_menu_fasttravel_conflict", "\A3\Data_F_Warlords\Data\preview_ft_conflict.jpg", localize "STR_A3_WL_menu_fasttravel_info"]};
_strategyArr pushBack ["FundsTransfer", WL2_fundsTransferCost, [], localize "STR_A3_WL_menu_fundstransfer", "\A3\Data_F_Warlords\Data\preview_cp_transfer.jpg", localize "STR_A3_WL_menu_fundstransfer_info"];
_strategyArr pushBack ["TargetReset", WL2_targetResetCost, [], localize "STR_A3_WL_menu_resetvoting", "\A3\Data_F_Warlords\Data\preview_ft_conflict.jpg", localize "STR_A3_WL_menu_resetvoting_info"];
...
missionNamespace setVariable [format ["WL2_purchasable_%1", _side], _sortedArray];