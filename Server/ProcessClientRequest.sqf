...
if (isMultiplayer) then {
	...
	private _senderArr = WL2_allWarlords select {getPlayerUID _x == _var};
...
_processTargetPos = {
	...
	if !(surfaceIsWater _targetPos) then {
		...
		if (count _nearSectorArr == 0) then {
			_targetPosFinalArr = [_sender, nil, FALSE, _sender] call WL2_fnc_findSpawnPositions;
		} else {
			_sector = _nearSectorArr # 0;
			_targetPosFinalArr = [_sector, nil, FALSE, if (_sender inArea (_sector getVariable "objectAreaComplete")) then {_sender}] call WL2_fnc_findSpawnPositions;
		};
	} else {
...
if !(isNull _sender) then {
	switch (_action) do {
		...
		case "requestAsset": {
			...
			if (_className isKindOf "Ship") then {
				...
				if (_className isKindOf "Air") then {
					...
					if (_isPlane) then {
						private _sector = ((_targetPos nearObjects ["Logic", 10]) select {count (_x getVariable ["WL2_runwaySpawnPosArr", []]) > 0}) # 0;
						private _taxiNodes = _sector getVariable "WL2_runwaySpawnPosArr";
						...
						[_asset, _sender] call WL2_fnc_sub_assetLanding;
					};
				} else {
					if (_isStatic) then {
		...
		case "requestAssetArray": {
			...
			_targetPos = call compile _targetPos;
			_purchaseList = missionNamespace getVariable format ["WL2_purchasable_%1", side group _sender];
			_classNames = [];
			_assets = [];
			...
			
			{
				...
				if (_isMan) then {
					...
					_asset setVariable ["WL2_deployPos", _targetPosFinal];
				...
				_assetVariable = call WL2_fnc_generateVariableName;
		...
		case "fundsTransfer": {
			...
			_recipientArr = WL2_allWarlords select {getPlayerUID _x == _recipient};
			_amount = parseNumber _amount;
			if (count _recipientArr > 0) then {
				_recipient = _recipientArr # 0;
				[_sender, -_amount] call WL2_fnc_fundsControl;
				[_recipient, _amount] call WL2_fnc_fundsControl;
			};
		};
		case "fastTravel": {
