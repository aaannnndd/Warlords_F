...
_purchase_transfer_amount ctrlSetText str (ceil ((player getVariable "WL2_funds") / 2));
...
{_purchase_transfer_units lbAdd name _x} forEach ((WL2_allWarlords select {isPlayer _x && side group _x == side group player}) - [player]);
...
[_purchase_transfer_units, _purchase_transfer_ok, _purchase_transfer_amount] spawn {
	...
	while {ctrlEnabled (_this # 1)} do {
		...
		if (_value <= (player getVariable "WL2_funds") && _value > 0) then {
			uiNamespace setVariable ["WL2_fundsTransferPossible", TRUE];
			(_this # 1) ctrlSetBackgroundColor _color;
			(_this # 1) ctrlSetTextColor [1, 1, 1, 1];
			(_this # 1) ctrlSetTooltip "";
		} else {
			uiNamespace setVariable ["WL2_fundsTransferPossible", FALSE];
		...
		lbClear (_this # 0);
		{(_this # 0) lbAdd name _x} forEach ((WL2_allWarlords select {isPlayer _x && side group _x == side group player}) - [player]);
		lbSort (_this # 0);
