...
switch (_event) do {
	case "init": {
		{
			_varName = format ["WL2_showHint_%1", _x];
		...
		while {!WL2_missionEnd} do {
			...
			if (WL2_showHint_maintenance) then {
				_hintText = _hintText + format [
					"%1<t size = '1.1' shadow = '0'>%2</t>",
					if (_hintText == "") then {""} else {"<br/><br/>"},
					format [
						localize "STR_WL2_hint_maintenance",
						"<t color = '#4bff58'>",
						"</t>"
					]
				];
			...
			if (WL2_showHint_targetResetVoting) then {
				_varNameVoting = format ["WL2_targetResetVotingSince_%1", WL2_playerSide];
				_warlords = WL2_allWarlords select {side group _x == WL2_playerSide};
	...
	default {
		_varName = format ["WL2_showHint_%1", _event];
